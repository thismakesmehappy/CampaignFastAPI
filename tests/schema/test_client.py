...
class TestClientCreate:
    def test_valid_minimal(self):
        client = ClientCreate(name=VALID_NAME)
    ...
    def test_valid_full(self):
        client = ClientCreate(name=VALID_NAME, email=VALID_EMAIL, notes="VIP", is_active=False)
    ...
    def test_name_too_long(self):
        with pytest.raises(ValidationError):
            ClientCreate(name="A" * (CAMPAIGN_NAME_MAX_LENGTH + 1))
...
class TestClientRead:
    def test_from_attributes(self):
        ...
        assert read.api_key == "generated-key"
        assert read.email == VALID_EMAIL
        assert read.is_active is True

    def test_api_key_exposed(self):
